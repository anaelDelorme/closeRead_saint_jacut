...
// Calculer l'étendue (extent) manuellement à partir de GeoJSON
function calculateExtent(geojson) {
    ...
    return [
        [Math.min(...xValues), Math.min(...yValues)], // Coin inférieur gauche
        [Math.max(...xValues), Math.max(...yValues)]  // Coin supérieur droit
    ];
}

franceExtent = calculateExtent(regs);
bretagneExtent = calculateExtent(bretagne);

// Define interpolations for extent values
interpolateXMin = d3.interpolateNumber(franceExtent[0][0], bretagneExtent[0][0]);
interpolateXMax = d3.interpolateNumber(franceExtent[1][0], bretagneExtent[1][0]);
interpolateYMin = d3.interpolateNumber(franceExtent[0][1], bretagneExtent[0][1]);
interpolateYMax = d3.interpolateNumber(franceExtent[1][1], bretagneExtent[1][1]);

// Use crScrollProgress for interpolation
progress = crProgressBlock;

// Interpolate extent values
x_min = interpolateXMin(progress);
x_max = interpolateXMax(progress);
y_min = interpolateYMin(progress);
y_max = interpolateYMax(progress);
...
En avant vers les Côtes-d'Armor ! Et sa côté d'Emeraude.
...
En novembre, la fraîcheur revient tout doucement, à 12°C, un peu de répit avant que l’hiver ne s’installe. [@cr-temperature-morning]{scale-by="1.3"}
...
Puis vient l’automne, qui ramène la température à 15°C, un air plus frais mais encore doux. [@cr-temperature-evening]{scale-by="1.3"} 
